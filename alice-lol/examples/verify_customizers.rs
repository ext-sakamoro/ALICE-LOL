...
fn main() {
    let cases = [
        ...
        Case { label: "tissue_box_cover (231, 116, 53)", lol: "tissue_box_cover(231, 116, 53)", expected: "hollow shell (< 0.60)" },
        Case { label: "storage_box (150, 100, 60)", lol: "storage_box(150, 100, 60)", expected: "hollow box (< 0.30)" },
        Case { label: "card_tray (63, 88, 12)", lol: "card_tray(63, 88, 12)", expected: "tray w/ notch (< 0.70)" },
        Case { label: "token_well (30, 4, 20)", lol: "token_well(30, 4, 20)", expected: "4 wells (< 0.70)" },
        Case { label: "wrench_holder (8, 19, 6)", lol: "wrench_holder(8, 19, 6)", expected: "6 slots (< 0.75)" },
        Case { label: "esp32_enclosure (60, 30, 20)", lol: "esp32_enclosure(60, 30, 20)", expected: "PCB enclosure (< 0.60)" },
        Case { label: "battery_18650_holder (4, 21, 6)", lol: "battery_18650_holder(4, 21, 6)", expected: "4 cylinder wells (< 0.75)" },
        Case { label: "toothbrush_holder (4, 12, 60)", lol: "toothbrush_holder(4, 12, 60)", expected: "4 wells (< 0.75)" },
        Case { label: "drill_bit_holder (2, 10, 8)", lol: "drill_bit_holder(2, 10, 8)", expected: "8 sized wells (< 0.75)" },
        Case { label: "pliers_rack (4, 150, 40)", lol: "pliers_rack(4, 150, 40)", expected: "4 slots (< 0.75)" },
        Case { label: "egg_tray (3, 4, 25)", lol: "egg_tray(3, 4, 25)", expected: "12 cups (< 0.55)" },
        Case { label: "cutting_board_rack (3, 12, 220)", lol: "cutting_board_rack(3, 12, 220)", expected: "3 slots (< 0.75)" },

        // ── Working reference (baseline for comparison) ──
        Case { label: "REF: pen_cup (30, 100)", lol: "pen_cup(30, 100)", expected: "hollow cylinder (~0.15-0.25)" },
        Case { label: "REF: gridfinity_bin(2,2,6)", lol: "gridfinity_bin(2, 2, 6)", expected: "hollow bin (< 0.35)" },
        Case { label: "REF: sticky_note_holder(80, 80, 30)", lol: "sticky_note_holder(80, 80, 30)", expected: "hollow (< 0.60)" },

        // ── Sanity: solid box (must be ~1.0) ──
        Case { label: "SANITY: box3d(50, 30, 20)", lol: "box3d(50, 30, 20)", expected: "SOLID (~1.0)" },
    ];
    ...
    for case in cases {
        ...
        for &(bbox, res) in probe_stages {
            let cfg = MarchingCubesConfig { resolution: res, ..Default::default() };
        ...
        let ratio = if aabb_vol > 0.0 { mesh_vol / aabb_vol } else { 0.0 };

        let flag = if ratio > 0.85 { " ← BOX-BUG" }
                   else if case.label.starts_with("SANITY") { "" }
                   else if case.label.starts_with("REF") { "" }
                   else if ratio > 0.75 { " ← suspicious" }
                   else { "" };
        ...
        if std::env::var("EXPORT_STL").is_ok() && !mesh.vertices.is_empty() {
            let name: String = case.label.chars()
    ...
    println!();
    println!("=== Summary ===");
    println!("Box-bug hits (ratio > 0.85, excluding SANITY solid): {}", box_bug_hits.len());
